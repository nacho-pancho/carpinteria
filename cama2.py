...
def crear_parrilla(ancho, largo, ancho_tabla, grosor_tabla):
    ...
    #
    pies = list()
    marco_parrilla_izq,pie = piezas.crear_placa("lado",ancho_tabla, largo-grosor_tabla, grosor_tabla,"lado_parrilla",material="pino")
    ...
    marco_parrilla_fre,pie = piezas.crear_placa("frente",ancho_tabla,ancho,grosor_tabla,"frente_parrilla",material="pino")
    pies.append(pie)
    marco_parrilla_fon     = marco_parrilla_fre.translate((0,largo,0))
    pies.append(pie)
    parrilla = (marco_parrilla_izq
    ...
    #
    tabla_parrilla,pie = piezas.crear_placa("horizontal",ancho, ancho_tabla, grosor_tabla,nombre="tabla_parrilla",material="pino")
    tabla_y0 = 0
...
def crear_patas(ancho, largo, grosor_tabla_parrilla, alto_pata, grosor_pata):
    ...
    pata_izq_fre,pie = piezas.crear_tabla("vertical",grosor_pata, alto_pata, grosor_pata,"pata",material="pino")
    pata_izq_fre = pata_izq_fre.translate((grosor_tabla_parrilla,grosor_tabla_parrilla,0))
    pies.append(pie)
...
parrilla, parr_pie = crear_parrilla(ancho_parrilla,largo_parrilla,ancho_tabla,grosor_tabla)
parrilla = parrilla.translate((0,0,alto_pata-ancho_tabla))
patas, pata_pie = crear_patas(ancho_parrilla,largo_parrilla,grosor_tabla, alto_pata, grosor_pata)
color_pino = cq.Color("cornsilk1")

objs = (cq.Assembly()
        .add(parrilla,color=color_pino)
        .add(patas,color=color_pino)
)
vis.show(objs)
