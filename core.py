...
class LayoutConstraints(JSONable):
    ...
    def __init__(self,
                 ...
                 preferred_size:Size=Size(), # undefined
                 padding:Padding=Padding(),
                 margin:Margin=Margin(),
                 weight:list[float]=[1,1,1],
                 alignment:list[str]=[CENTER,CENTER,CENTER]):
        
        self.preferred_size = copy.deepcopy(preferred_size)
 ...
 min_size {self.min_size} max_size {self.max_size} preferred_size {self.preferred_size} padding {self.padding}\
 {self.margin} weight {self.weight}\
 alignment {self.alignment}'''

    def to_dict(self):
        return {'min_size':self.min_size.to_dict(),
                'max_size':self.max_size.to_dict(),
                'preferred_size': self.preferred_size.to_dict(),
    ...
    def from_dict(d:dict):
        ...
        preferred_size = Size.from_dict(d['preferred_size'])
        padding = Padding.from_dict(d['padding'])
        margin = Margin.from_dict(d['margin'])
        weight = d['weight']
        alignment = d['alignment']
        return LayoutConstraints(min_size=min_size,
                                 max_size=max_size,
    ...
    def __init__(self, 
        ...
        for i in range(3):
            if fixed_size[i] is not None:
                self.constraints.min_size.dim[i] = self.constraints.max_size.dim[i] = fixed_size[i]
...
def simple_layout(_volume:Volume, part:Piece):
    logger = get_logger()
    logger.info(f'Layoing out part {piece} within  {_volume} with coinstraints {piece.constraints}')
...
class CompositePiece(Piece):
    ...
    def add_piece(self, piece:Piece,  position):
    ...
    def __str__(self):
        str = f'Composite {self.name} with layout {self.layout} and {len(self.parts)} parts:\n'
    ...
    def check(self):
        logger = get_logger()
        logger.info('Checking composite')
        if not super().check():
            return False
        for i,p in enumerate(self.parts):
            logger.info(f'Checking part {i}:')
            if not p.check():
                return False
        logger.info(f'Checking passed.')
        return True
